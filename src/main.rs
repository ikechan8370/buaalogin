...
use druid::widget::{FillStrat, Flex, Image, Label, SizedBox, TextBox};
use druid::{AppLauncher, Color, Data, Env, ImageBuf, Lens, LocalizedString, Menu, Widget, WidgetExt, WindowDesc, WindowId};
...
fn build_widget() -> Box<dyn Widget<AppState>> {
    let png_data = ImageBuf::from_data(include_bytes!("./assets/img.png")).unwrap();

    let mut img = Image::new(png_data).fix_size(500.0, 500.0);
    SizedBox::new(img).center().boxed()
